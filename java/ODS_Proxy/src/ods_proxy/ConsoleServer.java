 ...
 * @author tw5015
 */
public class ConsoleServer extends Thread implements TelnetServerListener {

    ODS_ProxyConsole console;
    int listeningPort;
    ...
    public ConsoleServer( ODS_ProxyConsole console, int port ) {
    ...
    public void ReceiveData( TelnetServer client, byte [] data, int length ) {
        try { console.WriteData( data, length ); } catch ( IOException e ) { e.printStackTrace(); }
    }
    ...
    public void OnSocketClosed( TelnetServer client ) {
        ...
        for (i = 0; i < clients.length; i++)
        {
            if (clients[i] != null && clients[i] == client) { clients[i] = null; }
        }
        }
    }
    ...
    public String GetClients() {
        String  text = "Active console client :\r\n";
        String  client = "";
